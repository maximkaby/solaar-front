...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bond, value, provider, networkID }: ICalcBondDetailsAsyncThunk, { dispatch }): Promise<IBondDetails> => {
    ...
    try {
      ...
      bondDiscount = (marketPrice * Math.pow(10, bond.tokenDecimals || 18) - Number(bondPrice.toString())) / Number(bondPrice.toString()); // 1 - bondPrice / (bondPrice * Math.pow(10, 9));
    } catch (e) {
    ...
    return {
      ...
      bondPrice: Number(bondPrice.toString()) / Math.pow(10, bond.tokenDecimals || 18),
...
export const bondAsset = createAsyncThunk(
  ...
  async ({ value, address, bond, networkID, provider, slippage }: IBondAssetAsyncThunk, { dispatch }) => {
    ...
    const valueInWei = String(
      Number(value) * Math.pow(10, bond.tokenDecimalsPrincipal || 18)
    );
    let balance;
