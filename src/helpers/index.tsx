...
import { gaia_usdc } from "./AllBonds";
import { JsonRpcSigner, StaticJsonRpcProvider } from "@ethersproject/providers";
import { IBaseAsyncThunk } from "src/slices/interfaces";
import { PairContract, RedeemHelper } from "../typechain";

export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const gaia_usdc_address = gaia_usdc.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(gaia_usdc_address, PairContractABI, provider) as PairContract;
...
export function prettyVestingPeriodTime(bondDetails: any) {
  const restTime = 1 - Number(bondDetails.vestingPercent) / 10**4;
  if (restTime < 0) {
    return "Fully Vested";
  }
  return prettifySeconds(restTime * bondDetails.vesting);
}
...
export function createData(name, rariry, quantity, value, type) {
  return { name, rariry, quantity, value, type };
}

export const getBondsCountString = (value: any) => {
  const stringLength = 4;
  const valueLength = String(value).length;
  return '0'.repeat(stringLength - valueLength) + value;
}