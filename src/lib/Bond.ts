...
export interface NetworkAddresses {
  [NetworkID.AVAX_MAINNET]: BondAddresses;
  [NetworkID.AVAX_TESTNET]: BondAddresses;
}

export interface Available {
  [NetworkID.AVAX_MAINNET]?: boolean;
  [NetworkID.AVAX_TESTNET]?: boolean;
}
...
export abstract class Bond {
  ...
  readonly tokenDecimalsPrincipal?: number;
...
export class LPBond extends Bond {
  ...
  constructor(lpBondOpts: LPBondOpts) {
    ...
    this.tokenDecimals = lpBondOpts.tokenDecimals;
  }
  async getTreasuryBalance(networkID: NetworkID, provider: StaticJsonRpcProvider) {
    ...
    let tokenUSD = (
      ...
      * (Number(markdown.toString()) / Math.pow(10,this.tokenDecimals || 18)
    );
...
export class StableBond extends Bond {
  ...
  constructor(stableBondOpts: StableBondOpts) {
    ...
    this.tokenDecimalsPrincipal = stableBondOpts.tokenDecimalsPrincipal;
    this.tokenDecimals = stableBondOpts.tokenDecimals;
  }
  ...
  async getTreasuryBalance(networkID: NetworkID, provider: StaticJsonRpcProvider) {
    ...
    return Number(tokenAmount.toString()) / Math.pow(10, this.tokenDecimalsPrincipal || 18);
  }
}
