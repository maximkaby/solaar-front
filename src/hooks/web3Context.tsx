...
import { Networks, RPC_URL, DEFAULT_NETWORK } from "src/constants";
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const rpcUrl = (RPC_URL as any)[chainID];
  ...
  const _checkNetwork = (otherChainID: number): boolean => {
    if (chainID !== otherChainID) {
      ...
      if (
        ...
        Number(otherChainID) === Networks.BSC
      ) {
        const rpcUrl = (RPC_URL as any)[otherChainID];
